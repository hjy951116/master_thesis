...
if __name__ == "__main__":
    ...
    for i in range (len(index)-1):
        ...
        diff = cv2.absdiff(flash1, noflash1) 
        cv2.imshow('diff', diff)  
        noflashcomp = noflash1
        ...
        diff3 = cv2.absdiff(noflash1, noflashcomp2)
        diff4 = cv2.absdiff(flashcomp, noflashcomp1)
        diff5 = cv2.absdiff(diff4, diff1)
        diff6 = cv2.absdiff(diff4, diff3)
        for r in range(0,flash.shape[0]):
            for c in range(0,flash.shape[1]):
                if diff2[r,c][...,0] >= 20 & diff1[r,c][...,0] >= 5:
                    noflashcomp[r,c] = noflashcomp1[r,c] + diff1[r,c]
        cv2.imshow('diff1',diff1)
        cv2.imshow('diff3',diff3)
        cv2.imshow('diff4',diff4)
        cv2.imshow('diff5',diff5)
        cv2.imshow('diff6',diff6)
